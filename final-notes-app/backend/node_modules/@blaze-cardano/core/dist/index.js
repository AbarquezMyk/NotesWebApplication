...
__export(index_exports, {
  ...
  NativeScripts: () => nativescript_exports,
  NetworkId: () => NetworkId,
  ...
  TxCBOR: () => TxCBOR,
  UTxOSelectionError: () => UTxOSelectionError,
  Value: () => Value,
  ...
  prettify: () => prettify,
  setInConwayEra: () => setInConwayEra2,
  sha2_256: () => sha2_256,
  sha3_256: () => sha3_2562,
...
var StakeDeregistration = C.Serialization.StakeDeregistration;
...
var import_blakejs = __toESM(require("blakejs"));
var bip39 = __toESM(require("@scure/bip39"));
var import_ed25519 = require("@noble/curves/ed25519");
var import_english = require("@scure/bip39/wordlists/english");
function getPaymentAddress(address2) {
  const bech = address2.toBech32();
...
function blake2b_256(data) {
  return Hash32ByteBase162(import_blakejs.default.blake2bHex(fromHex(data), void 0, 32));
}
function blake2b_224(data) {
  return Hash28ByteBase162(import_blakejs.default.blake2bHex(fromHex(data), void 0, 28));
}
...
// src/nativescript.ts
var nativescript_exports = {};
__export(nativescript_exports, {
  address: () => address,
  after: () => after,
  allOf: () => allOf,
  anyOf: () => anyOf,
  atLeastNOfK: () => atLeastNOfK,
  before: () => before,
  justAddress: () => justAddress
});
function keyhashFromAddress(address2, networkId) {
  const props = addressFromBech32(address2).getProps();
  if (props.networkId != networkId) {
    throw new Error(
      `Address ${address2} is on network ${props.networkId} not on network ${networkId}`
    );
  }
  if (props.paymentPart?.type == CredentialType.KeyHash && props.paymentPart.hash) {
    return Ed25519KeyHashHex2(props.paymentPart.hash);
  } else {
    throw new Error(`Address ${address2} is not a payment address`);
  }
}
function allOf(...addresses) {
  return NativeScript.newScriptAll(new ScriptAll(addresses));
}
function anyOf(...addresses) {
  return NativeScript.newScriptAny(new ScriptAny(addresses));
}
function atLeastNOfK(n, ...addresses) {
  return NativeScript.newScriptNOfK(new ScriptNOfK(addresses, n));
}
function justAddress(address2, networkId) {
  return NativeScript.newScriptPubkey(
    new ScriptPubkey(keyhashFromAddress(address2, networkId))
  );
}
function before(slot) {
  return NativeScript.newTimelockExpiry(new TimelockExpiry(slot));
}
function after(slot) {
  return NativeScript.newTimelockStart(new TimelockStart(slot));
}
function address(ns, networkId) {
