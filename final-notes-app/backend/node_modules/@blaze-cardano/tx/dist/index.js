const require_value = require('./value-BHaQy3IY.js');
const __blaze_cardano_core = require_value.__toESM(require("@blaze-cardano/core"));
...
function wideSelection(inputs, dearth) {
	...
	let acc = new __blaze_cardano_core.Value(0n);
	for (let j = 0; j < 30; j++) {
		const diff = require_value.sub(dearth, acc);
		const goal = require_value.positives(diff);
		if (require_value.assetTypes(goal) <= 1) break;
		else {
			let bestStep = [
				0,
				new __blaze_cardano_core.Value(0n),
				-1
			];
			for (let i = 0; i < availableInputs.length; i += 1) {
				const iValue = availableInputs[i].output().amount();
				const iImprove = require_value.positives(require_value.sub(goal, iValue));
				const iIntersect = require_value.intersect(goal, iValue);
				const rating = require_value.assetTypes(goal) - require_value.assetTypes(iImprove) + iIntersect / 10;
			...
			if (bestStep[2] == -1) throw new __blaze_cardano_core.UTxOSelectionError("wide", dearth, availableInputs, selectedInputs, bestStep);
			selectedInputs.push(...availableInputs.splice(bestStep[2], 1));
			acc = require_value.merge(acc, bestStep[1]);
		}
	}
	return {
		selectedInputs,
...
function deepSelection(inputs, dearth) {
	...
	for (let j = 0; j < 30; j++) {
		const diff = require_value.sub(dearth, acc);
		const goal = require_value.positives(diff);
		if (require_value.assetTypes(goal) == 0) break;
		else {
			const searchAsset = require_value.assets(goal)[0];
			let bestStep = [
				0n,
				new __blaze_cardano_core.Value(0n),
				-1
			...
			if (bestStep[2] == -1) throw new __blaze_cardano_core.UTxOSelectionError("deep", dearth, availableInputs, selectedInputs, bestStep);
			selectedInputs.push(...availableInputs.splice(bestStep[2], 1));
			acc = require_value.merge(acc, bestStep[1]);
		}
	}
...
const micahsSelector = (inputs, dearth) => {
	...
	const remainingDearth = require_value.positives(require_value.sub(dearth, wideResult.selectedValue));
	const deepResult = deepSelection(wideResult.leftoverInputs, remainingDearth);
	const finalDearth = require_value.positives(require_value.sub(remainingDearth, deepResult.selectedValue));
	const selectedInputs = [...wideResult.selectedInputs, ...deepResult.selectedInputs];
	if (!require_value.empty(finalDearth)) throw new __blaze_cardano_core.UTxOSelectionError("final", finalDearth, inputs, selectedInputs);
	return {
		selectedInputs,
		selectedValue: require_value.merge(wideResult.selectedValue, deepResult.selectedValue),
...
function getScriptSize(script) {
	const cborReader = new __blaze_cardano_core.CborReader(script.toCbor());
...
function computeScriptData(redeemers, datums, usedCostModels) {
	...
	const scriptDataHash = (0, __blaze_cardano_core.blake2b_256)(hashedData);
	return {
...
*/
const getAuxiliaryDataHash = (data) => (0, __blaze_cardano_core.blake2b_256)(data.toCbor());
/**
...
const assertPaymentsAddress = (address) => {
	...
	if (props.paymentPart.type == __blaze_cardano_core.CredentialType.ScriptHash) throw new Error("assertPaymentsAddress: address payment credential cannot be a script hash!");
};
/**
...
const assertLockAddress = (address) => {
	...
	if (props.paymentPart.type != __blaze_cardano_core.CredentialType.ScriptHash) throw new Error("assertLockAddress: address payment credential must be a script hash!");
};
/**
...
var TxBuilder = class {
	...
	*/
	constructor(params, tracing = false) {
		...
		this.redeemers = __blaze_cardano_core.Redeemers.fromCore([]);
		...
		(0, __blaze_cardano_core.setInConwayEra)(true);
		this.params = params;
		this.body = new __blaze_cardano_core.TransactionBody(__blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.TransactionInput.fromCore), [], 0n, void 0);
	}
	/**
	...
	*/
	get burnAddress() {
		if (!this._burnAddress) this._burnAddress = (0, __blaze_cardano_core.getBurnAddress)(this.networkId);
	...
	*/
	addReferenceInput(utxo) {
		const referenceInputs = this.body.referenceInputs() ?? __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.TransactionInput.fromCore);
	...
	*/
	addInput(utxo, redeemer, unhashDatum) {
		...
		for (const redeemer$1 of redeemers) if (redeemer$1.tag() == __blaze_cardano_core.RedeemerTag.Spend && redeemer$1.index() >= BigInt(insertIdx)) redeemer$1.setIndex(redeemer$1.index() + 1n);
		...
		if (redeemer !== void 0) {
			if (key.type == __blaze_cardano_core.CredentialType.KeyHash) throw new Error("addInput: Cannot spend with redeemer for KeyHash credential!");
			...
			redeemers.push(__blaze_cardano_core.Redeemer.fromCore({
				index: insertIdx,
				purpose: __blaze_cardano_core.RedeemerPurpose["spend"],
		...
		} else if (key.type == __blaze_cardano_core.CredentialType.ScriptHash) this.requiredNativeScripts.add(key.hash);
		else this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(key.hash));
		const scriptRef = utxo.output().scriptRef();
		if (scriptRef) {
			this.scriptScope.add(scriptRef);
			this.scriptSeen.add(scriptRef.hash());
	...
	*/
	addMint(policy, assets$1, redeemer) {
		const insertIdx = insertSorted(this.consumedMintHashes, (0, __blaze_cardano_core.PolicyIdToHash)(policy));
		const mint = this.body.mint() ?? /* @__PURE__ */ new Map();
		for (const asset of mint.keys()) if (__blaze_cardano_core.AssetId.getPolicyId(asset) == policy) throw new Error("addMint: Duplicate policy!");
		for (const [key, amount] of assets$1.entries()) mint.set(__blaze_cardano_core.AssetId.fromParts(policy, key), amount);
		this.body.setMint(mint);
		if (redeemer) {
			this.requiredPlutusScripts.add((0, __blaze_cardano_core.PolicyIdToHash)(policy));
			const redeemers = [...this.redeemers.values()];
			for (const redeemer$1 of redeemers) if (redeemer$1.tag() == __blaze_cardano_core.RedeemerTag.Mint && redeemer$1.index() >= BigInt(insertIdx)) redeemer$1.setIndex(redeemer$1.index() + 1n);
			redeemers.push(__blaze_cardano_core.Redeemer.fromCore({
				index: insertIdx,
				purpose: __blaze_cardano_core.RedeemerPurpose["mint"],
				data: redeemer.toCore(),
				executionUnits: {
					memory: this.params.maxExecutionUnitsPerTransaction.memory,
					steps: this.params.maxExecutionUnitsPerTransaction.steps
			...
			this.redeemers.setValues(redeemers);
		} else this.requiredNativeScripts.add((0, __blaze_cardano_core.PolicyIdToHash)(policy));
		return this;
	}
	/**
	* This methods calculates the minimum ada required for a transaction output.
	...
	*/
	checkAndAlterOutput(output) {
		...
		while (coin < minAda) {
			...
			output = new __blaze_cardano_core.TransactionOutput(output.address(), amount);
	...
	*/
	payLovelace(address, lovelace, datum) {
		...
		const paymentAddress = (0, __blaze_cardano_core.getPaymentAddress)(address);
		const datumData = typeof datum == "object" ? datum.toCore() : void 0;
		const datumHash = typeof datum == "string" ? datum : void 0;
		this.addOutput(__blaze_cardano_core.TransactionOutput.fromCore({
	...
	*/
	lockLovelace(address, lovelace, datum, scriptReference) {
		return this.lockAssets(address, new __blaze_cardano_core.Value(lovelace), datum, scriptReference);
	}
	/**
	...
	*/
	lockAssets(address, value, datum, scriptReference) {
		...
		const paymentAddress = (0, __blaze_cardano_core.getPaymentAddress)(address);
		return this.addOutput(__blaze_cardano_core.TransactionOutput.fromCore({
			address: paymentAddress,
			value: value.toCore(),
			datum: datumData,
			datumHash,
	...
	*/
	deployScript(script, address = this.burnAddress) {
		const out = new __blaze_cardano_core.TransactionOutput(address, new __blaze_cardano_core.Value(0n));
	...
	*/
	buildFinalWitnessSet(signatures) {
		const tw = new __blaze_cardano_core.TransactionWitnessSet();
		const scriptLookup = {};
		...
		for (const requiredScriptHash of this.requiredPlutusScripts) {
			...
			if (script?.asNative() == void 0) {
				...
				if (lang == 1) this.usedLanguages[__blaze_cardano_core.PlutusLanguageVersion.V1] = true;
				else if (lang == 2) this.usedLanguages[__blaze_cardano_core.PlutusLanguageVersion.V2] = true;
				else if (lang == 3) this.usedLanguages[__blaze_cardano_core.PlutusLanguageVersion.V3] = true;
		...
		if (sn.length != 0) {
			const cborSet = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.NativeScript.fromCore);
			cborSet.setValues(sn);
			tw.setNativeScripts(cborSet);
		}
		if (s1.length != 0) {
			const cborSet = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.PlutusV1Script.fromCore);
			cborSet.setValues(s1);
			tw.setPlutusV1Scripts(cborSet);
		}
		if (s2.length != 0) {
			const cborSet = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.PlutusV2Script.fromCore);
			cborSet.setValues(s2);
			tw.setPlutusV2Scripts(cborSet);
		}
		if (s3.length != 0) {
			const cborSet = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.PlutusV3Script.fromCore);
			cborSet.setValues(s3);
			tw.setPlutusV3Scripts(cborSet);
		}
		tw.setVkeys(__blaze_cardano_core.CborSet.fromCore(signatures, __blaze_cardano_core.VkeyWitness.fromCore));
		tw.setRedeemers(this.redeemers);
		const plutusData = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.PlutusData.fromCore);
	...
	*/
	updateRequiredWitnesses() {
		...
		totalInputs.forEach((input) => {
			...
			if (!key) return;
			if (key.type == __blaze_cardano_core.CredentialType.ScriptHash) {
		...
		for (const cert of this.body.certs()?.values() || []) switch (cert.kind()) {
			case 0:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asStakeRegistration().stakeCredential().hash));
				break;
			case 1:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asStakeDeregistration().stakeCredential().hash));
				break;
			case 2:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asStakeDelegation().stakeCredential().hash));
				break;
			case 3:
				this.requiredWitnesses.add((0, __blaze_cardano_core.Ed25519PublicKeyHex)(cert.asPoolRegistration().poolParameters().operator()));
				break;
			case 4:
				this.requiredWitnesses.add((0, __blaze_cardano_core.Ed25519PublicKeyHex)(cert.asPoolRetirement().poolKeyHash()));
				break;
			case 16:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asRegisterDelegateRepresentativeCert().credential().hash));
				break;
			case 17:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asUnregisterDelegateRepresentativeCert().credential().hash));
				break;
			case 18:
				this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(cert.asUpdateDelegateRepresentativeCert().credential().hash));
				break;
		}
	}
	/**
	...
	*/
	buildPlaceholderWitnessSet() {
		...
		const placeholderSignatures = Array.from({ length: this.requiredWitnesses.size + this.additionalSigners }, (_, i) => [(0, __blaze_cardano_core.Ed25519PublicKeyHex)(i.toString(16).padStart(64, "0")), (0, __blaze_cardano_core.Ed25519SignatureHex)(i.toString(16).padStart(128, "0"))]);
	...
	*/
	getAssetSurplus() {
		...
		let inputValue = new __blaze_cardano_core.Value(withdrawalAmount);
		let outputValue = new __blaze_cardano_core.Value(bigintMax(this.fee, this.minimumFee) + (this.body.donation() ?? 0n));
		const mintValue = new __blaze_cardano_core.Value(0n, this.body.mint());
		for (const input of this.body.inputs().values()) {
			...
			inputValue = require_value.merge(inputValue, utxo.output().amount());
		}
		for (const output of this.body.outputs().values()) outputValue = require_value.merge(outputValue, output.amount());
		for (const cert of this.body.certs()?.values() || []) switch (cert.kind()) {
			case 0:
				outputValue = require_value.merge(outputValue, new __blaze_cardano_core.Value(BigInt(this.params.stakeKeyDeposit)));
				break;
			case 1:
				inputValue = require_value.merge(inputValue, new __blaze_cardano_core.Value(BigInt(this.params.stakeKeyDeposit)));
				break;
			case 3:
				if (this.params.poolDeposit) outputValue = require_value.merge(outputValue, new __blaze_cardano_core.Value(BigInt(this.params.poolDeposit)));
				break;
			case 4:
				if (this.params.poolDeposit) inputValue = require_value.merge(inputValue, new __blaze_cardano_core.Value(BigInt(this.params.poolDeposit)));
				break;
		}
		const tilt = require_value.merge(require_value.merge(inputValue, require_value.negate(outputValue)), mintValue);
		return tilt;
	}
	/**
	...
	*/
	getScriptData(tw) {
		const usedCostModels = new __blaze_cardano_core.Costmdls();
		for (let i = 0; i <= Object.keys(this.usedLanguages).length; i++) if (this.usedLanguages[i]) {
			...
			usedCostModels.insert(new __blaze_cardano_core.CostModel(i, cm));
		}
	...
	*/
	adjustChangeOutput(surplusValue) {
		...
		if (require_value.empty(surplusValue)) {
		...
		const newChangeOutput = new __blaze_cardano_core.TransactionOutput(this.changeAddress, require_value.merge(changeOutput.amount(), surplusValue));
	...
	getOrCreateChangeOutput() {
		if (this.changeOutputIndex === void 0) {
			...
			this.addOutput(this.checkAndAlterOutput(new __blaze_cardano_core.TransactionOutput(this.changeAddress, require_value.zero())));
		}
		const outputs = this.body.outputs();
	...
	splitOutputIfNeeded(output) {
		...
		const outputs = [];
		let nextOutput = new __blaze_cardano_core.TransactionOutput(output.address(), require_value.zero());
		let remainingLovelace = output.amount().coin();
		for (const [asset, quantity] of Array.from(output.amount().multiasset()?.entries() ?? [])) {
			const prospectiveValue = require_value.merge(nextOutput.amount(), require_value.makeValue(0n, [asset, quantity]));
			const prospectiveOutput = new __blaze_cardano_core.TransactionOutput(nextOutput.address(), prospectiveValue);
			...
			if (prospectiveSize > this.params.maxValueSize * .9) {
				...
				nextOutput = new __blaze_cardano_core.TransactionOutput(output.address(), require_value.makeValue(0n, [asset, quantity]));
			} else nextOutput = prospectiveOutput;
		}
		if (!require_value.empty(nextOutput.amount())) {
			const correctedOutput = this.checkAndAlterOutput(nextOutput);
			outputs.push(correctedOutput);
			remainingLovelace -= correctedOutput.amount().coin();
		}
	...
	*/
	calculateFees() {
		...
		const draft_tx = new __blaze_cardano_core.Transaction(this.body, draft_ws, this.auxiliaryData);
	...
	*/
	prepareCollateral(params = { useCoinSelection: true }) {
		...
		if (!useCoinSelection) {
			...
			const totalValue = require_value.sum(providedCollateral.map((pc) => pc.output().amount()));
			const requiredCollateral = calculateRequiredCollateral(this.fee, this.params.collateralPercentage);
			const collateralReturn = require_value.merge(totalValue, require_value.negate(new __blaze_cardano_core.Value(requiredCollateral)));
			this.trace(`Preparing collateral...`, {
				requiredCollateral,
				providedCollateral: totalValue.coin(),
				collateralReturn: collateralReturn.coin()
			});
			const tis = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.TransactionInput.fromCore);
			...
			this.body.setCollateralReturn(this.checkAndAlterOutput(new __blaze_cardano_core.TransactionOutput(this.collateralChangeAddress ?? this.changeAddress, collateralReturn)));
		} else {
			const requiredCollateral = calculateRequiredCollateral(this.body.fee(), this.params.collateralPercentage);
			this.body.setTotalCollateral(requiredCollateral);
			let providedCollateral = require_value.sum([...this.collateralUtxos.values()].map((c) => c.output().amount()));
			let collateralReturn = require_value.sub(providedCollateral, new __blaze_cardano_core.Value(requiredCollateral));
			...
			if (providedCollateral.coin() < requiredCollateral) {
				const cleanInputs = [...this.utxos.values()].filter((utxo) => {
					if (utxo.output().address().getProps().paymentPart?.type === __blaze_cardano_core.CredentialType.ScriptHash) return false;
					const newOutput = __blaze_cardano_core.TransactionOutput.fromCbor((0, __blaze_cardano_core.HexBlob)(utxo.output().toCbor()));
				...
				const { selectedInputs } = this.coinSelector(cleanInputs.filter((utxo) => {
					...
					return true;
				}), new __blaze_cardano_core.Value(requiredCollateral), Number(this.fee));
				...
				const collateralSet = __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.TransactionInput.fromCore);
				collateralSet.setValues(selectedInputs.map((si) => si.input()));
				providedCollateral = require_value.sum(selectedInputs.map((si) => si.output().amount()));
				collateralReturn = require_value.sub(providedCollateral, new __blaze_cardano_core.Value(requiredCollateral));
			...
			const collateralOutput = new __blaze_cardano_core.TransactionOutput(this.collateralChangeAddress ?? this.changeAddress, collateralReturn);
	...
	* */
	toCbor() {
		...
		return new __blaze_cardano_core.Transaction(this.body, tw, this.auxiliaryData).toCbor();
	}
	/**
	...
	async complete(params = { useCoinSelection: true }) {
		...
		while (iterations < 10) {
			...
			if (this.redeemers.size() > 0 && this.body.inputs().size() > 0 && this.body.outputs().length > 0) {
				const tx = new __blaze_cardano_core.Transaction(this.body, witnessSet, this.auxiliaryData);
				try {
			...
			for (const utxo of this.utxos.values()) {
				allScriptInputs = utxo.output().address().getProps().paymentPart?.type !== __blaze_cardano_core.CredentialType.KeyHash;
			...
			for (const utxo of this.utxos.values()) {
				...
				if (!hasInput && (utxo.output().address().getProps().paymentPart?.type !== __blaze_cardano_core.CredentialType.ScriptHash || allScriptInputs)) spareInputs.push(utxo);
			}
			...
			const deficit = require_value.negatives(surplusAndDeficits);
			...
			if (!require_value.empty(surplusAndDeficits)) {
				this.trace(`Transaction is imbalanced`, surplusAndDeficits.toCore());
				if (useCoinSelection && !require_value.empty(deficit)) {
					this.trace(`Running coin selection to satisfy imbalance`);
					const selectionResult = this.coinSelector(spareInputs, require_value.negate(deficit));
					...
					if (selectionResult.selectedInputs.length > 0) for (const input of selectionResult.selectedInputs) this.addInput(input);
						...
						const [inputWithLeastMultiAssets] = spareInputs.reduce(([minInput, minMultiAssetCount], currentInput) => {
							const currentMultiAssetCount = require_value.assetTypes(currentInput.output().amount());
							return currentMultiAssetCount < minMultiAssetCount ? [currentInput, minMultiAssetCount] : [minInput, require_value.assetTypes(minInput.output().amount())];
						}, [spareInputs[0], require_value.assetTypes(spareInputs[0].output().amount())]);
					...
					surplusAndDeficits = require_value.merge(surplusAndDeficits, selectionResult.selectedValue);
					if (!require_value.empty(require_value.negatives(surplusAndDeficits))) throw new Error("Unreachable! CoinSelection is expected to throw an error if it can't reach the goal, but it returned a selection result that didn't satisfy the goal.");
				}
		...
		return new __blaze_cardano_core.Transaction(this.body, finalWitnessSet, this.auxiliaryData);
	}
	/**
	...
	*/
	addDelegation(delegator, poolId, redeemer) {
		const stakeDelegation = {
			__typename: __blaze_cardano_core.CertificateType.StakeDelegation,
			stakeCredential: delegator.toCore(),
			poolId
		};
		const delegationCertificate = __blaze_cardano_core.Certificate.newStakeDelegation(__blaze_cardano_core.StakeDelegation.fromCore(stakeDelegation));
		const certs = this.body.certs() ?? __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.Certificate.fromCore);
		...
		if (delegatorCredential.type == __blaze_cardano_core.CredentialType.ScriptHash) if (redeemer) {
			this.requiredPlutusScripts.add(delegatorCredential.hash);
			const redeemers = [...this.redeemers.values()];
			for (const redeemer$1 of redeemers) if (redeemer$1.tag() == __blaze_cardano_core.RedeemerTag.Reward && redeemer$1.index() >= BigInt(insertIdx)) redeemer$1.setIndex(redeemer$1.index() + 1n);
			redeemers.push(__blaze_cardano_core.Redeemer.fromCore({
				index: insertIdx,
				purpose: __blaze_cardano_core.RedeemerPurpose["certificate"],
				data: redeemer.toCore(),
				executionUnits: {
					memory: this.params.maxExecutionUnitsPerTransaction.memory,
					steps: this.params.maxExecutionUnitsPerTransaction.steps
		...
		else this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(delegatorCredential.hash));
		return this;
	}
	/**
	...
	*/
	delegate(poolId, redeemer) {
		...
		this.addDelegation(__blaze_cardano_core.Credential.fromCore(credential), poolId, redeemer);
		return this;
	}
	/**
	...
	*/
	addRegisterStake(credential) {
		const stakeRegistration = new __blaze_cardano_core.StakeRegistration(credential.toCore());
		const registrationCertificate = __blaze_cardano_core.Certificate.newStakeRegistration(stakeRegistration);
		const certs = this.body.certs() ?? __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.Certificate.fromCore);
	...
	*/
	addDeregisterStake(credential, redeemer) {
		const stakeDeregistration = new __blaze_cardano_core.StakeDeregistration(credential.toCore());
		const deregistrationCertificate = __blaze_cardano_core.Certificate.newStakeDeregistration(stakeDeregistration);
		const certs = this.body.certs() ?? __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.Certificate.fromCore);
	...
	*/
	addWithdrawal(address, amount, redeemer) {
		const withdrawalHash = __blaze_cardano_core.Address.fromBech32(address).getProps().paymentPart?.hash;
		...
		if (redeemer) {
			...
			redeemers.push(__blaze_cardano_core.Redeemer.fromCore({
				index: insertIdx,
				purpose: __blaze_cardano_core.RedeemerPurpose["withdrawal"],
				data: redeemer.toCore(),
				executionUnits: {
					memory: this.params.maxExecutionUnitsPerTransaction.memory,
					steps: this.params.maxExecutionUnitsPerTransaction.steps
			...
			const key = __blaze_cardano_core.Address.fromBech32(address).getProps().paymentPart;
			if (!key) throw new Error("addWithdrawal: The RewardAccount provided does not have an associated stake credential.");
			if (key.type == __blaze_cardano_core.CredentialType.ScriptHash) this.requiredNativeScripts.add(key.hash);
			else this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(key.hash));
		}
		return this;
	}
	...
	*/
	addRequiredSigner(signer) {
		const signers = this.body.requiredSigners() ?? __blaze_cardano_core.CborSet.fromCore([], __blaze_cardano_core.Hash.fromCore);
		this.requiredWitnesses.add((0, __blaze_cardano_core.HashAsPubKeyHex)(__blaze_cardano_core.Hash28ByteBase16.fromEd25519KeyHashHex(signer)));
		const values = [...signers.values()];
		values.push(__blaze_cardano_core.Hash.fromCore(signer));
	...
	*/
	setMetadata(metadata) {
		if (!this.auxiliaryData) this.auxiliaryData = new __blaze_cardano_core.AuxiliaryData();
...
const selectUTxOs = (utxos, totalRequired, includeUTxOsWithScriptRef = false) => {
	...
	for (const utxo of utxos) {
		...
		for (const [unit, amount] of assetsRequired) {
			const assetAmount = unit === "lovelace" ? utxo.output().amount().coin() : utxo.output().amount().multiasset()?.get((0, __blaze_cardano_core.AssetId)(unit));
			if (assetAmount) {
...
const calculateExtraLovelace = (leftoverAssets, address, coinsPerUtxoByte) => {
	const output = new __blaze_cardano_core.TransactionOutput(address, leftoverAssets);
	const minLovelace = calculateMinAda(output, coinsPerUtxoByte);
	const currentLovelace = leftoverAssets.coin();
	return currentLovelace >= minLovelace ? new __blaze_cardano_core.Value(0n) : new __blaze_cardano_core.Value(minLovelace - currentLovelace);
};
/**
...
* @returns
*/
const recursive = (inputs, requiredAssets, externalAssets = new __blaze_cardano_core.Value(0n), coinsPerUtxoByte = __blaze_cardano_core.hardCodedProtocolParams.coinsPerUtxoByte) => {
	...
	const selectedAssets = require_value.sum(selected.map((s) => s.output().amount()));
	let availableAssets = require_value.sub(selectedAssets, requiredAssets);
	availableAssets = require_value.sum([availableAssets, externalAssets]);
	...
	while (extraLovelace.coin() > 0n) {
		...
		const extraSelectedAssets = require_value.sum(extraSelected.map((v) => v.output().amount()));
		selected = [...selected, ...extraSelected];
		availableAssets = require_value.sum([availableAssets, extraSelectedAssets]);
...
* @returns
*/
const hvfSelector = (inputs, collectedAssets, preliminaryFee = 0, externalAssets = new __blaze_cardano_core.Value(0n), coinsPerUtxoByte = __blaze_cardano_core.hardCodedProtocolParams.coinsPerUtxoByte) => {
	const requiredAssets = new __blaze_cardano_core.Value(collectedAssets.coin() + externalAssets.coin() + BigInt(preliminaryFee));
	const nonRequiredAssets = new __blaze_cardano_core.Value(0n);
	externalAssets.multiasset()?.forEach((amount, id) => {
		...
		else nonRequiredAssets.setMultiasset(new Map([[(0, __blaze_cardano_core.AssetId)(id), amount]]));
		else if (requiredMultiAsset) requiredAssets.multiasset().set(id, requiredMultiAsset + amount);
		else requiredAssets.setMultiasset(new Map([[(0, __blaze_cardano_core.AssetId)(id), amount]]));
	});
...
require_value.__export(coinSelectors_exports, {
	hvfSelector: () => hvfSelector,
	micahsSelector: () => micahsSelector
});

//#endregion
Object.defineProperty(exports, 'CoinSelector', {
  enumerable: true,
  get: function () {
    return coinSelectors_exports;
  }
});
...
Object.defineProperty(exports, 'Value', {
  enumerable: true,
  get: function () {
    return require_value.value_exports;
  }
});
exports.assertLockAddress = assertLockAddress;
exports.assertPaymentsAddress = assertPaymentsAddress;
exports.assertValidOutput = assertValidOutput;
exports.bigintMax = bigintMax;
exports.calculateMinAda = calculateMinAda;
exports.calculateReferenceScriptFee = calculateReferenceScriptFee;
exports.calculateRequiredCollateral = calculateRequiredCollateral;
exports.computeScriptData = computeScriptData;
exports.getAuxiliaryDataHash = getAuxiliaryDataHash;
exports.getScriptSize = getScriptSize;
exports.insertSorted = insertSorted;
exports.isEqualInput = isEqualInput;
exports.isEqualOutput = isEqualOutput;
exports.isEqualUTxO = isEqualUTxO;
exports.makeValue = require_value.makeValue;
exports.sortLargestFirst = sortLargestFirst;
exports.stringifyBigint = stringifyBigint;