import { CborReader, CborReaderState, CborWriter, ConstrPlutusData, HexBlob, NativeScript, PlutusData, PlutusList, PlutusV1Script, PlutusV2Script, PlutusV3Script, Script, fromHex, toHex } from "@blaze-cardano/core";
...
var UPLCDecoder = class UPLCDecoder extends Parser {
	...
	*/
	#decodeBool() {
		return PlutusData.newConstrPlutusData(new ConstrPlutusData(this.popBit() == 0 ? 0n : 1n, new PlutusList())).toCore();
	}
	/**
...
var UPLCEncoder = class extends Encoder {
	...
	*/
	encodeData(type, data) {
		...
		else if (type === "Data") this.encodeByteString(fromHex(PlutusData.fromCore(data).toCbor()));
...
function stripCbor(cbor) {
	...
	while (cborReader.peekState() != CborReaderState.ByteString) cborReader.readTag();
...
function cborToScript(cbor, type) {
	if (type === "Native") return Script.newNativeScript(NativeScript.fromCbor(HexBlob(cbor)));
	else {
		const cborReader = new CborReader(HexBlob(cbor));
		while (cborReader.peekState() != CborReaderState.ByteString) cborReader.readTag();
		const cborBytes = cborReader.readByteString();
		...
		cborWriter.writeByteString(cborBytes);
		const cborHex = cborWriter.encodeAsHex();
		if (type === "PlutusV1") return Script.newPlutusV1Script(new PlutusV1Script(cborHex));
		else if (type === "PlutusV2") return Script.newPlutusV2Script(new PlutusV2Script(cborHex));
		else if (type == "PlutusV3") return Script.newPlutusV3Script(new PlutusV3Script(cborHex));
		else throw new Error("Unsupported script type");
	}
}

//#endregion
export { UPLCDecoder, UPLCEncoder, applyParams, applyParamsToScript, cborToScript };