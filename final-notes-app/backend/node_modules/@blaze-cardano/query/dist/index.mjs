import { Address, AddressType, AssetId, Datum, DatumHash, ExUnits, Hash28ByteBase16, HexBlob, NativeScript, NetworkId, PlutusData, PlutusLanguageVersion, PlutusV1Script, PlutusV2Script, PlutusV3Script, RedeemerPurpose, RedeemerTag, Redeemers, SLOT_CONFIG_NETWORK, Script, Slot, TransactionId, TransactionInput, TransactionOutput, TransactionUnspentOutput, Value, fromHex, getBurnAddress, hardCodedProtocolParams } from "@blaze-cardano/core";
...
var Provider = class {
	...
	*/
	async resolveScriptRef(script, address = getBurnAddress(this.network)) {
		...
		if (script instanceof Script) script = script.hash();
	...
	*/
	getSlotConfig() {
		switch (this.networkName) {
			case "cardano-mainnet": return SLOT_CONFIG_NETWORK.Mainnet;
			case "cardano-preprod": return SLOT_CONFIG_NETWORK.Preprod;
			case "cardano-preview": return SLOT_CONFIG_NETWORK.Preview;
	...
	*/
	unixToSlot(unix_millis) {
		...
		return Slot((Number(unix_millis) - slotConfig.zeroTime) / slotConfig.slotLength + slotConfig.zeroSlot);
	}
	/**
...
var Blockfrost = class extends Provider {
	constructor(params) {
		...
		super(network == "cardano-mainnet" ? NetworkId.Mainnet : NetworkId.Testnet, network);
	...
	*/
	async getUnspentOutputs(address, filter) {
		...
		let page = 1;
		const bech32 = address instanceof Address ? address.toBech32() : new Address({
	...
	*/
	async getUnspentOutputByNFT(nft) {
		const query = `/assets/${AssetId.getPolicyId(nft) + AssetId.getAssetName(nft)}/addresses`;
		const json = await fetch(`${this.url}${query}`, { headers: this.headers() }).then((resp) => resp.json());
	...
	async resolveUnspentOutputs(txIns) {
		...
		for (const txIn of txIns) {
			...
			for (const blockfrostUTxO of response.outputs) {
				...
				const buildTxUnspentOutput = this.buildTransactionUnspentOutput(Address.fromBech32(blockfrostUTxO.address));
	...
	*/
	async postTransactionToChain(tx) {
		...
		return TransactionId(await response.json());
	}
	/**
	...
	*/
	...
	async getScriptRef(scriptHash) {
		...
		switch (type) {
			case "plutusV1":
				this.scriptCache.set(scriptHash, Script.newPlutusV1Script(new PlutusV1Script(cbor)));
				break;
			case "plutusV2":
				this.scriptCache.set(scriptHash, Script.newPlutusV2Script(new PlutusV2Script(cbor)));
				break;
			case "plutusV3":
				this.scriptCache.set(scriptHash, Script.newPlutusV3Script(new PlutusV3Script(cbor)));
				break;
	...
	*/
	buildTransactionUnspentOutput(address) {
		return async (blockfrostUTxO) => {
			const txIn = new TransactionInput(TransactionId(blockfrostUTxO.tx_hash), BigInt(blockfrostUTxO.output_index));
			...
			const datum = blockfrostUTxO.inline_datum ? Datum.newInlineData(PlutusData.fromCbor(HexBlob(blockfrostUTxO.inline_datum))) : blockfrostUTxO.data_hash ? Datum.newDataHash(DatumHash(blockfrostUTxO.data_hash)) : void 0;
			if (datum) txOut.setDatum(datum);
			if (blockfrostUTxO.reference_script_hash) txOut.setScriptRef(await this.getScriptRef(Hash28ByteBase16(blockfrostUTxO.reference_script_hash)));
			return new TransactionUnspentOutput(txIn, txOut);
		};
	}
};
const fromBlockfrostLanguageVersion = (x) => {
	if (x == "PlutusV1") return PlutusLanguageVersion.V1;
	else if (x == "PlutusV2") return PlutusLanguageVersion.V2;
	else if (x == "PlutusV3") return PlutusLanguageVersion.V3;
...
var Kupmios = class Kupmios extends Provider {
	...
	*/
	constructor(kupoUrl, ogmios) {
		super(ogmios.url.includes("mainnet-v6") ? NetworkId.Mainnet : NetworkId.Testnet, ogmios.url.includes("mainnet-v6") ? "cardano-mainnet" : ogmios.url.includes("preprod-v6") ? "cardano-preprod" : ogmios.url.includes("preview-v6") ? "cardano-preview" : "unknown");
	...
	*/
	async _getUnspentOutputs(prefix, postfix) {
		...
		return await Promise.all(result.map(async (utxo) => {
			...
			const output = new TransactionOutput(address, new Value(coins, Object.keys(assets).reduce((map, unit) => {
				map.set(AssetId(unit), BigInt(assets[unit]));
				return map;
			}, /* @__PURE__ */ new Map())));
			if (utxo.datum_hash) {
				const datum = utxo.datum_type === "hash" ? new Datum(utxo.datum_hash, void 0) : new Datum(void 0, await this.resolveDatum(utxo.datum_hash));
			...
			return new TransactionUnspentOutput(new TransactionInput(TransactionId(transactionId), outputIndex), output);
		}));
	}
	/**
	...
	*/
	async resolveScript(scriptHash) {
		...
		switch (result.language) {
			case "native": return Script.newNativeScript(NativeScript.fromCbor(result.script));
			case "plutus:v1": return Script.newPlutusV1Script(new PlutusV1Script(result.script));
			case "plutus:v2": return Script.newPlutusV2Script(new PlutusV2Script(result.script));
			case "plutus:v3": return Script.newPlutusV3Script(new PlutusV3Script(result.script));
...
//#endregion
export { Blockfrost, Kupmios, Provider, fromBlockfrostLanguageVersion, purposeToTag };